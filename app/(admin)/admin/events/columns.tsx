...
export interface EventColumnsOptions {
  /** Base admin path for the edit link, e.g. "/admin/events" or "/admin/pozvanky". */
  basePath: string
  /** Whether to show the organizer badge (external invitations only; AKH carry none). */
  showOrganizer: boolean
}

export function getEventColumns({ basePath, showOrganizer }: EventColumnsOptions): ColumnDef<Event>[] {
  return [
  {
    accessorKey: "title",
    header: ({ column }) => {
    ...
    cell: ({ row }) => {
        ...
        const showBadge = showOrganizer && !!event.organizer_id
        return (
  ...
  },
  {
    ...
    header: () => <div className="hidden md:block">Místo</div>,
    cell: ({ row }) => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const event = row.original
      return <ActionCell event={event} basePath={basePath} />
    },
  },
  ]
}
...
function ActionCell({ event, basePath }: { event: Event; basePath: string }) {
    ...
    return (
        <>
            ...
            <div className="flex items-center gap-0 sm:gap-2">
                <Link href={`${basePath}/${event.id}`} className="sm:hidden">
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-pencil"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>
                        <span className="sr-only">Upravit</span>
                    </Button>
                </Link>
                <Link href={`${basePath}/${event.id}`} className="hidden sm:block">
